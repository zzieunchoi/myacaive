...
헤드 예시
<title>: 브라우저 상단 타이틀
<meta>: 문서 레벨 메타데이터 요소 - 데이터를 설명하는 데이터
...
1) 요소: HTML에서 시작태그와 종료태그로 이루어진 모든 명령어들, 태그명령어 종류들

2) 태그: 요소의 하나로서 시작태그와 종료태그로 이루어져 있습니다.

3) 속성: 조금 더 구제화된 명령어, 다양한 효과 부여
...
그 후에는 규칙을 세우기 위해 새로운 태그가 등장(HTML5에서 의미론적 요소를 담은 태그의 등장)
...
**id**(고유 식별자), **class**(공통적인 식별자), data-*, **style**, title, tabindex 등등
...
* 대표적인 속성
  ...
  * required, readonly(읽기만 가능), autofocus, autocomplete, disabled(value값이외에 건들일수 없음)
...
<label for = "normal">37도미만</label>
<input type = "radio" name = "body-heat" id = "warning" value = "warning">
<label for = "warning">37도이상</label>
```
